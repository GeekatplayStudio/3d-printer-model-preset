...
from app.models import AnalysisLevel, Cavity, CavityConfidenceLevel, GeometryAnalysis, Island, MeshHealthReport, MeshRepairOutcome, ModelSoftwareKind, ModelSourceMetadata, SliceArea, UseCase
...
_GENERIC_METADATA_PATTERN = re.compile(r"^([A-Za-z][A-Za-z0-9 _/.-]{1,40})\s*[:=]\s*(.+)$")
_SOFTWARE_VALUE_PATTERN = re.compile(r"^(.*?)(?:\s+v?(\d+(?:\.\d+)+(?:[^\s]*)?))$", re.IGNORECASE)
_AUTHOR_PATTERNS = (
    re.compile(r"(?:author|artist|designer|creator|maker)\s*[:=]\s*(.+)$", re.IGNORECASE),
    re.compile(r"(?:created by|made by|designed by|modeled by|modelled by)\s+(.+)$", re.IGNORECASE),
)
_SOFTWARE_PATTERNS = (
    re.compile(r"(?:software|application|tool|generator)\s*[:=]\s*(.+)$", re.IGNORECASE),
    re.compile(r"(?:generated by|exported by|created with|made with|saved by)\s+(.+)$", re.IGNORECASE),
)
_CREATED_AT_PATTERNS = (
    re.compile(r"(?:created(?: at)?|timestamp|generated(?: at)?|exported(?: at)?|date)\s*[:=]\s*(.+)$", re.IGNORECASE),
)
_SOFTWARE_SIGNATURES: tuple[tuple[str, ModelSoftwareKind, tuple[str, ...]], ...] = (
    ("Meshy", "ai", ("meshy",)),
    ("Tripo", "ai", ("tripo", "tripo ai")),
    ("Sloyd", "ai", ("sloyd",)),
    ("Luma", "ai", ("luma ai", "genie")),
    ("Shap-E", "ai", ("shap-e",)),
    ("Point-E", "ai", ("point-e",)),
    ("Trellis", "ai", ("trellis", "microsoft trellis")),
    ("Rodin", "ai", ("rodin", "hyperhuman")),
    ("CSM", "ai", ("common sense machines", "csm ai", "csm")),
    ("Masterpiece X", "ai", ("masterpiece x", "masterpiecex")),
    ("Kaedim", "ai", ("kaedim",)),
    ("Blender", "dcc", ("blender",)),
    ("ZBrush", "dcc", ("zbrush",)),
    ("Autodesk Maya", "dcc", ("autodesk maya", "maya")),
    ("3ds Max", "dcc", ("3ds max", "3dsmax")),
    ("Nomad Sculpt", "dcc", ("nomad sculpt",)),
    ("Cinema 4D", "dcc", ("cinema 4d", "c4d")),
    ("Fusion 360", "cad", ("fusion 360", "fusion360")),
    ("SolidWorks", "cad", ("solidworks",)),
    ("Onshape", "cad", ("onshape",)),
    ("FreeCAD", "cad", ("freecad",)),
    ("OpenSCAD", "cad", ("openscad",)),
    ("Rhino", "cad", ("rhino", "rhinoceros")),
    ("Shapr3D", "cad", ("shapr3d",)),
    ("RealityCapture", "scan", ("realitycapture",)),
    ("Metashape", "scan", ("metashape", "agisoft")),
    ("Revopoint", "scan", ("revopoint",)),
    ("Polycam", "scan", ("polycam",)),
    ("Cura", "slicer", ("cura", "ultimaker cura")),
    ("PrusaSlicer", "slicer", ("prusaslicer",)),
    ("OrcaSlicer", "slicer", ("orcaslicer",)),
    ("Bambu Studio", "slicer", ("bambu studio",)),
    ("Lychee", "slicer", ("lychee",)),
    ("CHITUBOX", "slicer", ("chitubox",)),
    ("Simplify3D", "slicer", ("simplify3d",)),
    ("PreForm", "slicer", ("preform", "formlabs preform")),
)
_AI_HINTS = (
    " ai ",
    "ai-generated",
...
_THREE_MF_NS = "http://schemas.microsoft.com/3dmanufacturing/core/2015/02"
...
def _normalize_detected_datetime(value: str) -> str:
...
def _stringify_metadata_value(value: object) -> str | None:
    if value is None:
        return None
...
def _normalize_metadata_line(text: str) -> str:
    ...
    if lowered.startswith("solid "):
        return cleaned[6:].strip()
    return cleaned


def _split_metadata_segments(text: str) -> list[str]:
    return [segment.strip() for segment in re.split(r"[;|]", text) if segment.strip()]


def _match_metadata_value(text: str, patterns: tuple[re.Pattern[str], ...]) -> str | None:
...
def _decode_printable_text(value: bytes) -> str:
    decoded = value.decode("utf-8", errors="ignore").replace("\x00", " ")
    return " ".join(decoded.split())


def _detect_file_format(path: Path, prefix: bytes) -> str:
...
def _detect_stl_encoding(path: Path, prefix: bytes) -> str:
    size = path.stat().st_size
    if size >= 84:
        with path.open("rb") as handle:
            header = handle.read(84)
        if len(header) >= 84:
            declared_faces = struct.unpack("<I", header[80:84])[0]
            if 84 + (declared_faces * 50) == size:
                return "binary"
    stripped = prefix.lstrip()
    if stripped.startswith(b"solid"):
        return "ascii"
...
def _extract_header_fragments(path: Path, stl_encoding: str, prefix: bytes) -> tuple[str | None, list[str]]:
    if stl_encoding == "binary":
        with path.open("rb") as handle:
            header = handle.read(80)
        header_text = _decode_printable_text(header)
        if not header_text:
            return None, []
    ...
    for raw_line in text.splitlines():
        ...
        if lowered.startswith(("facet ", "outer loop", "vertex ", "endloop", "endfacet", "endsolid")):
            break
        lines.append(stripped)
        if len(lines) >= 12:
            break
    header_text = " | ".join(lines) if lines else None
    return header_text, lines


def _split_software_value(value: str) -> tuple[str, str | None]:
    cleaned = value.strip()
    match = _SOFTWARE_VALUE_PATTERN.match(cleaned)
    if not match:
        return cleaned, None
    name = match.group(1).strip(" -")
    version = match.group(2).strip()
    return name or cleaned, version or None


def _detect_software_signature(text: str) -> tuple[str | None, ModelSoftwareKind, list[str]]:
    lowered = f" {text.lower()} "
    hits: list[str] = []
    for label, kind, tokens in _SOFTWARE_SIGNATURES:
        for token in tokens:
            if token.lower() in lowered:
                hits.append(label)
                return label, kind, hits
    return None, "unknown", hits


def _pick_metadata_field(extracted_fields: dict[str, str], *candidates: str) -> str | None:
    for candidate in candidates:
        value = extracted_fields.get(candidate)
        if value:
            return value
    for key, value in extracted_fields.items():
        for candidate in candidates:
            if key.endswith(f"_{candidate}") and value:
                return value
    return None


def _flatten_metadata_fields(prefix: str, value: object, extracted_fields: dict[str, str], limit: int = 80) -> None:
    if len(extracted_fields) >= limit:
        return
    if isinstance(value, dict):
        for key, nested in value.items():
            next_key = _normalize_metadata_key(str(key))
            if not next_key:
                continue
            _flatten_metadata_fields(f"{prefix}_{next_key}" if prefix else next_key, nested, extracted_fields, limit=limit)
            if len(extracted_fields) >= limit:
                return
        return
    if isinstance(value, list):
        for index, nested in enumerate(value[:5]):
            _flatten_metadata_fields(f"{prefix}_{index}" if prefix else str(index), nested, extracted_fields, limit=limit)
            if len(extracted_fields) >= limit:
                return
        return
    text = _stringify_metadata_value(value)
    if prefix and text and prefix not in extracted_fields:
        extracted_fields[prefix] = text


def _metadata_warning_message(file_format: str) -> str:
    if file_format == "stl":
        return "STL files rarely preserve reliable author, software, or creation timestamps; missing metadata is normal."
    if file_format == "glb":
        return "GLB metadata depends on the exporter. Author, tool, and timestamp fields are optional."
    if file_format == "3mf":
        return "3MF supports richer metadata than STL, but author, tool, and timestamp fields are still optional."
    return "Embedded authoring metadata is optional for this file format."


def _filesystem_created_at(stat_result: os.stat_result) -> str | None:
    birthtime = getattr(stat_result, "st_birthtime", None)
    if birthtime is not None:
        return _to_iso8601(float(birthtime))
    if os.name == "nt":
        return _to_iso8601(float(stat_result.st_ctime))
    return None
...
def _build_source_metadata(
    *,
    ...
    author = author or _pick_metadata_field(extracted_fields, "author", "artist", "designer", "creator", "maker")
    software = software or _pick_metadata_field(
        extracted_fields,
        "software",
        "application",
        "tool",
    ...
    embedded_created_at = embedded_created_at or _pick_metadata_field(
        extracted_fields,
        "created",
        "created_at",
        "creation_date",
    ...
    if embedded_created_at:
        embedded_created_at = _normalize_detected_datetime(embedded_created_at)

    combined_parts = [raw_metadata_text, author, software, *list(extracted_fields.values())[:40]]
    combined_text = " | ".join(str(part).strip() for part in combined_parts if str(part).strip())
    detected_software_name, detected_kind, _ = _detect_software_signature(combined_text)
    if software:
        software, software_version = _split_software_value(software)
    else:
        software = detected_software_name
        software_version = None

    software_kind: ModelSoftwareKind = detected_kind
    if software and software_kind == "unknown":
        _, software_kind, _ = _detect_software_signature(software)

    lowered_text = f" {combined_text.lower()} "
    ai_detection_basis: list[str] = []
    likely_ai_generated: bool | None = None
    for hint in _AI_HINTS:
        if hint in lowered_text:
            ai_detection_basis.append(f"Detected '{hint.strip()}' in embedded metadata text.")
    if software_kind == "ai":
        likely_ai_generated = True
        if software:
            ai_detection_basis.append(f"Detected AI-oriented generator signature '{software}'.")
    elif ai_detection_basis:
        likely_ai_generated = True
    elif software_kind in {"cad", "dcc", "scan", "slicer"} and software:
        likely_ai_generated = False
        ai_detection_basis.append(f"Detected non-AI tool signature '{software}'.")

    combined_warnings = list(warnings or [])
    if not any((author, software, embedded_created_at)):
        combined_warnings.append(_metadata_warning_message(file_format))
    if embedded_created_at is None:
        combined_warnings.append(
            "No embedded creation timestamp was found. Filesystem timestamps reflect the saved file, not necessarily the original authoring time."
        )
    if likely_ai_generated is None:
        combined_warnings.append("AI origin cannot be determined reliably from embedded file metadata alone.")
    ...
    return ModelSourceMetadata(
        ...
        embedded_created_at=embedded_created_at,
        filesystem_created_at=_filesystem_created_at(stat_result),
        filesystem_modified_at=_to_iso8601(float(stat_result.st_mtime)),
        likely_ai_generated=likely_ai_generated,
        ai_detection_basis=ai_detection_basis,
        extracted_fields=extracted_fields,
        warnings=combined_warnings,
    )


def _extract_stl_source_metadata(path: Path, stat_result: os.stat_result, prefix: bytes) -> ModelSourceMetadata:
    stl_encoding = _detect_stl_encoding(path, prefix)
    raw_metadata_text, header_lines = _extract_header_fragments(path, stl_encoding, prefix)
    extracted_fields: dict[str, str] = {}
    author: str | None = None
    software: str | None = None
    embedded_created_at: str | None = None

    for fragment in header_lines:
        normalized = _normalize_metadata_line(fragment)
        if not normalized:
            continue
        for segment in _split_metadata_segments(normalized):
            generic_match = _GENERIC_METADATA_PATTERN.match(segment)
            if generic_match:
                key = _normalize_metadata_key(generic_match.group(1))
                value = generic_match.group(2).strip()
                if key and value and key not in extracted_fields:
                    extracted_fields[key] = value
            author = author or _match_metadata_value(segment, _AUTHOR_PATTERNS)
            software = software or _match_metadata_value(segment, _SOFTWARE_PATTERNS)
            detected_created = _match_metadata_value(segment, _CREATED_AT_PATTERNS)
            if embedded_created_at is None and detected_created:
                embedded_created_at = detected_created
...
def _extract_glb_json(path: Path) -> dict[str, object]:
    with path.open("rb") as handle:
        data = handle.read()
    if len(data) < 20 or data[:4] != b"glTF":
        raise ValueError("File does not contain a valid GLB header.")

    _, total_length = struct.unpack_from("<II", data, 4)
...
def _extract_glb_source_metadata(path: Path, stat_result: os.stat_result) -> ModelSourceMetadata:
    document = _extract_glb_json(path)
    extracted_fields: dict[str, str] = {}
    raw_fragments: list[str] = []
    asset = document.get("asset") if isinstance(document.get("asset"), dict) else {}
    if asset:
        _flatten_metadata_fields("asset", asset, extracted_fields)
        for key in ("generator", "copyright", "version", "minVersion"):
            value = asset.get(key)
            text = _stringify_metadata_value(value)
            if text:
                raw_fragments.append(f"{key}={text}")
    extras = document.get("extras") if isinstance(document.get("extras"), dict) else {}
    if extras:
        _flatten_metadata_fields("extras", extras, extracted_fields)
    for collection_name in ("scenes", "nodes", "meshes", "materials"):
        collection = document.get(collection_name)
        if not isinstance(collection, list):
            continue
        for index, item in enumerate(collection[:5]):
            if not isinstance(item, dict):
                continue
            extras = item.get("extras")
            if isinstance(extras, dict):
                _flatten_metadata_fields(f"{collection_name}_{index}", extras, extracted_fields)

    raw_metadata_text = " | ".join(raw_fragments[:12]) if raw_fragments else None
    author = _pick_metadata_field(extracted_fields, "author", "creator", "designer", "artist")
    software = _pick_metadata_field(
        extracted_fields,
        "asset_generator",
        "software",
        "application",
    ...
    embedded_created_at = _pick_metadata_field(
        extracted_fields,
        "created",
        "created_at",
        "timestamp",
...
def _extract_3mf_source_metadata(path: Path, stat_result: os.stat_result) -> ModelSourceMetadata:
    try:
        with ZipFile(path) as archive:
            model_name = next((name for name in archive.namelist() if name.lower().endswith(".model")), None)
            if model_name is None:
                raise ValueError("3MF package does not contain a .model entry.")
            model_xml = archive.read(model_name)
    except BadZipFile as exc:
        raise ValueError("3MF package is not a valid ZIP archive.") from exc
    ...
    if unit:
        ...
        raw_fragments.append(f"unit={unit}")
    language = root.attrib.get("{http://www.w3.org/XML/1998/namespace}lang")
    if language:
        extracted_fields["language"] = language
    for metadata_node in root.findall(f"{{{_THREE_MF_NS}}}metadata"):
        name = _normalize_metadata_key(metadata_node.attrib.get("name", "metadata"))
        value = " ".join("".join(metadata_node.itertext()).split())
        if name and value:
            extracted_fields[name] = value
            raw_fragments.append(f"{name}={value}")
    ...
    return _build_source_metadata(
        ...
        raw_metadata_text=" | ".join(raw_fragments[:12]) if raw_fragments else None,
        extracted_fields=extracted_fields,
        author=_pick_metadata_field(extracted_fields, "designer", "author", "artist", "creator"),
        software=_pick_metadata_field(extracted_fields, "application", "creationapplication", "producer", "software"),
        embedded_created_at=_pick_metadata_field(extracted_fields, "creation_date", "created", "created_at", "timestamp"),
    )


def _extract_model_source_metadata(file_path: str) -> ModelSourceMetadata:
    path = Path(file_path)
    file_format = _detect_file_format(path, path.read_bytes()[: min(_METADATA_READ_BYTES, path.stat().st_size)] if path.exists() and path.is_file() else b"")
    if not path.exists() or not path.is_file():
        return ModelSourceMetadata(
            file_format=file_format,
            encoding="unknown",
            file_size_bytes=0,
            warnings=["Source file was not available for metadata inspection."],
        )
    stat = path.stat()
    with path.open("rb") as handle:
        prefix = handle.read(_METADATA_READ_BYTES)
    file_format = _detect_file_format(path, prefix)
    if file_format == "glb":
        return _extract_glb_source_metadata(path, stat)
    if file_format == "3mf":
        return _extract_3mf_source_metadata(path, stat)
    return _extract_stl_source_metadata(path, stat, prefix)
...
def analyze_geometry(
    ...
    source_metadata = _extract_model_source_metadata(file_path)
    timings_ms["inspect_metadata"] = _elapsed_ms(started)
